...
bool st_run_vector_tests() {
  ...
  vector_t vector;
  if (!st_assert_true(vector_init(&vector, 2, 3.0f), S("Vector Test Setup Allocation Failed"), S(""))) return false;
  ...
  if (!st_assert_equals_uX(3, vector.grow_factor, S("Vector Test \"Basic Add\" (PT7) Failed"), S("Vector Test \"Basic Add\" Passed"))) return false;
  ...
  if (!st_assert_equals_uX(3, vector.grow_factor, S("Vector Test \"Overflow\" (PT7) Failed"), S("Vector Test \"Overflow\" Passed"))) return false;
